...
final class MetalRenderer: NSObject, MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        ...
        let shouldUpdateParticles: Bool
        if let state = simulationEngine?.state {
            shouldUpdateParticles = (state != .idle) || enableIdleChaotic
        } else {
            shouldUpdateParticles = enableIdleChaotic
        }
    ...
    private func validateStructLayouts() {
        ...
        assert(particleStride % 16 == 0, "Particle layout not aligned to 16 bytes - stride \(particleStride)")
    }
}
