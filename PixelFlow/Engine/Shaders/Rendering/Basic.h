...
fragment float4 fragmentParticle(
    ...
    // Создаем круглую форму с мягкими краями (улучшенное сглаживание)
    float alpha = 1.0 - smoothstep(1.0 - PARTICLE_EDGE_SOFTNESS, 1.0, dist);

    // Дополнительное сглаживание для устранения пиксельной сетки
    alpha = pow(alpha, 0.8); // Немного смягчаем края
