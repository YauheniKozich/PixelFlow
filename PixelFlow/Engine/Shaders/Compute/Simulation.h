//
//  Simulation.h - СОСТОЯНИЯ СИМУЛЯЦИИ: Пульс системы PixelFlow
//  ============================================================
//
//  ЭТОТ ФАЙЛ - "ГОСУДАРСТВО" СИСТЕМЫ!
//  Здесь определяются состояния симуляции и правила переходов.
//
//  Представь симуляцию как человека:
//  - IDLE: спит спокойно
//  - CHAOTIC: танцует безумно
//  - COLLECTING: собирает вещи
//  - COLLECTED: отдыхает после работы
//  - LIGHTNING_STORM: в экстазе от молний
//
//  Здесь же константы, лимиты и вспомогательные функции.
//  Все, что нужно для координации между CPU и GPU.
//
//  Автор: Yauheni Kozich
//  Создан: 31.10.25
//  Обновлен: 2025-01-10 - Добавлены подробные русские комментарии
//
//  📝 ОПИСАНИЕ: Создано маленьким, но не глупым ИИ под чутким руководством человека
//  🤖 ИИ: Помогал с формулировками и структурой комментариев
//  👨‍💻 Человек: Определял технические детали и архитектурные решения
//

#ifndef Simulation_h
#define Simulation_h

#include <metal_stdlib>
using namespace metal;

// ============================================================================
// СОСТОЯНИЯ СИМУЛЯЦИИ - "НАСТРОЕНИЯ" СИСТЕМЫ
// ============================================================================

/*
    ЭТИ КОНСТАНТЫ - ОСНОВА ЛОГИКИ!
    Они ДОЛЖНЫ точно совпадать с enum в ParticleSystem.swift!

    Каждое состояние определяет:
    - Как двигаются частицы
    - Какие эффекты применяются
    - Как ведет себя освещение
    - Какие переходы возможны
*/

// 😴 РЕЖИМ ОЖИДАНИЯ - система спит
constant int SIMULATION_STATE_IDLE = 0;

/*
    Частицы слегка шевелятся, минимальное освещение.
    Энергия низкая, активность минимальная.
    Используется когда пользователь ничего не делает.
*/

// 🌪️ ХАОТИЧНЫЙ РЕЖИМ - танец безумия
constant int SIMULATION_STATE_CHAOTIC = 1;

/*
    Частицы кружат в вихре, получают импульсы, хаотично двигаются.
    Полная энергия, яркие эффекты, пульсирующее освещение.
    "Танцы до упаду" для частиц!
*/

// 🎯 РЕЖИМ СБОРА - частицы летят к целям
constant int SIMULATION_STATE_COLLECTING = 2;

/*
    Частицы плавно движутся к своим позициям на изображении.
    Спокойное, контролируемое движение.
    Завершается автоматически когда все частицы на месте.
*/

// ✅ СОБРАННОЕ СОСТОЯНИЕ - цель достигнута
constant int SIMULATION_STATE_COLLECTED = 3;

/*
    Все частицы на своих местах, движение остановлено.
    Стабильное изображение, теплое освещение.
    "Миссия выполнена!"
*/

// ⚡ ЭЛЕКТРИЧЕСКАЯ БУРЯ - молнии и хаос
constant int SIMULATION_STATE_LIGHTNING_STORM = 4;

/*
    Электрические поля, вспышки, турбулентность!
    Частицы дергаются от "молний", меняют цвета на электрические.
    Самый драматичный и энергичный режим.
*/

// ============================================================================
// ПОМОЩНИКИ ДЛЯ АНАЛИЗА СОСТОЯНИЙ - "ПСИХОЛОГИЯ" СИСТЕМЫ
// ============================================================================

/*
    Эти функции помогают понять "характер" текущего состояния.
    Они используются для принятия решений о поведении частиц.
*/

/*
    АКТИВНАЯ ЛИ СИМУЛЯЦИЯ?

    В idle режиме частицы почти не двигаются - это "пассивный" режим.
    Во всех остальных - активно танцуют.
*/
static inline bool isActiveSimulationState(int state) {
    return state != SIMULATION_STATE_IDLE;
}

/*
    ЭНЕРГИЧНЫЙ ЛИ РЕЖИМ?

    Некоторые режимы очень активные: хаос и буря требуют много энергии.
    Сбор - более спокойный процесс.
*/
static inline bool isEnergeticState(int state) {
    return state == SIMULATION_STATE_CHAOTIC ||
           state == SIMULATION_STATE_LIGHTNING_STORM;
}

/*
    СВЯЗАН ЛИ С СБОРОМ ЧАСТИЦ?

    Два состояния относятся к сбору: процесс сбора и завершенный результат.
*/
static inline bool isCollectionState(int state) {
    return state == SIMULATION_STATE_COLLECTING ||
           state == SIMULATION_STATE_COLLECTED;
}

/*
    МАСШТАБ ВРЕМЕНИ ДЛЯ КАЖДОГО СОСТОЯНИЯ

    В разных режимах время течет по-разному:
    - В idle: медленнее (сонное состояние)
    - В chaotic: нормально
    - В collecting: чуть медленнее для плавности
    - В collected: остановлено
*/
static inline float getStateTimeScale(int state) {
    switch (state) {
        case SIMULATION_STATE_IDLE:
            return 0.2;    // Замедленное время (сон)
        case SIMULATION_STATE_CHAOTIC:
            return 1.0;    // Нормальное время
        case SIMULATION_STATE_COLLECTING:
            return 0.8;    // Чуть медленнее для контроля
        case SIMULATION_STATE_COLLECTED:
            return 0.0;    // Остановлено (заморозка)
        default:
            return 1.0;    // По умолчанию - нормальная скорость
    }
}

// ============================================================================
// ДЕФОЛТНЫЕ ПАРАМЕТРЫ - "ФАБРИЧНЫЕ НАСТРОЙКИ"
// ============================================================================

/*
    Эти значения используются когда параметры не заданы явно.
    Они подобраны для стабильной и красивой работы.
*/

// ВРЕМЕННЫЕ ИНТЕРВАЛЫ - ритм симуляции
constant float DEFAULT_DELTA_TIME = 0.016666667;    // 60 FPS (1/60 секунды)
constant float MIN_DELTA_TIME = 0.0001;             // Минимальный шаг (защита от деления на 0)
constant float MAX_DELTA_TIME = 0.1;                // Максимальный шаг (защита от "телепортации")

// Алиасы для обратной совместимости (старый код использует эти имена)
#define DEFAULT_DT DEFAULT_DELTA_TIME
#define MIN_DT MIN_DELTA_TIME
#define MAX_DT MAX_DELTA_TIME

// ПАРАМЕТРЫ СБОРА - как частицы находят свои места
constant float COLLECTION_SPEED_BASE = 0.005;       // Базовая скорость движения к цели
constant float COLLECTION_THRESHOLD_DEFAULT = 0.99; // 99% частиц должны быть собраны

// ФИЗИЧЕСКИЕ КОНСТАНТЫ - реализм движения
constant float BOUNCE_DAMPING_DEFAULT = 0.8;        // Потеря скорости при отскоке
constant float VELOCITY_DAMPING_DEFAULT = 0.95;     // Общее замедление скорости

// РАЗМЕРЫ ЧАСТИЦ - от точки до звездочки
constant float MIN_PARTICLE_SIZE_DEFAULT = 1.5;     // Минимальный размер (пиксели)
constant float MAX_PARTICLE_SIZE_DEFAULT = 8.0;     // Максимальный размер (пиксели)

// МАТЕМАТИЧЕСКИЕ КОНСТАНТЫ - основа тригонометрии
constant float PI = 3.141592654;           // Пи (π)
constant float TWO_PI = 6.283185307;       // 2π (полный круг)
constant float HALF_PI = 1.570796327;      // π/2 (90 градусов)

// Алиас для совместимости (старый код использует M_PI_F)
#define M_PI_F PI

// ЗАЩИТНЫЕ КОНСТАНТЫ - от катастроф
#define MIN_VECTOR_LENGTH 0.0001     // Минимальная длина вектора (защита от 0)
#define MAX_FLOAT_VALUE 1e10         // Максимальное значение float
#define MIN_SCREEN_SIZE 0.0001       // Минимальный размер экрана

// ЖИЗНЕННЫЙ ЦИКЛ ЧАСТИЦ - флаги состояний
#define PARTICLE_ALIVE 0.0           // Частица жива и активна
#define PARTICLE_COLLECTED -1.0      // Частица собрана и заморожена

// ============================================================================
// ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ - ИНСТРУМЕНТЫ РАЗРАБОТЧИКА
// ============================================================================

static inline int getSimulationStateValue(int state) {
    return state; // Константы уже имеют правильные значения
}

static inline bool canTransitionToState(int currentState, int targetState) {
    // В IDLE можно перейти из любого состояния (стоп/сброс)
    if (targetState == SIMULATION_STATE_IDLE) {
        return true;
    }

    // Проверяем допустимые переходы для каждого состояния
    switch (currentState) {
        case SIMULATION_STATE_IDLE:
            // Из отдыха можно только начать хаос
            return targetState == SIMULATION_STATE_CHAOTIC;

        case SIMULATION_STATE_CHAOTIC:
            // Из хаоса можно начать сбор или бурю
            return targetState == SIMULATION_STATE_COLLECTING ||
                   targetState == SIMULATION_STATE_LIGHTNING_STORM;

        case SIMULATION_STATE_COLLECTING:
            // Сбор автоматически завершается в collected
            return targetState == SIMULATION_STATE_COLLECTED;

        case SIMULATION_STATE_COLLECTED:
            // После сбора можно начать новый хаос
            return targetState == SIMULATION_STATE_CHAOTIC;

        case SIMULATION_STATE_LIGHTNING_STORM:
            // Из бури возвращаемся в хаос
            return targetState == SIMULATION_STATE_CHAOTIC;

        default:
            // Неизвестное состояние - запрещаем переход
            return false;
    }
}

static inline int getStateDebugValue(int state) {
    return state;
}

static inline float calculateSafeDeltaTime(float requestedDeltaTime) {
    // Проверка на NaN или бесконечность
    if (!isfinite(requestedDeltaTime)) {
        return DEFAULT_DELTA_TIME;
    }

    // Ограничиваем допустимым диапазоном
    return clamp(requestedDeltaTime, MIN_DELTA_TIME, MAX_DELTA_TIME);
}

// Частица собрана? (life отрицательное)
static inline bool isParticleCollected(float life) {
    return life < 0.0;
}

// Отметить частицу как собранную
static inline void markParticleAsCollected(thread float& life) {
    life = PARTICLE_COLLECTED;
}

// Сбросить частицу в живое состояние
static inline void markParticleAsAlive(thread float& life) {
    life = PARTICLE_ALIVE;
}


constant int BOUNDARY_TYPE_BOUNCE = 0;
constant int BOUNDARY_TYPE_TOROIDAL = 1;
constant int BOUNDARY_TYPE_CLAMP = 2;


constant int INTEGRATION_METHOD_EULER = 0;
constant int INTEGRATION_METHOD_VERLET = 1;


#endif /* Simulation_h */
